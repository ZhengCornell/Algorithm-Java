...
class CommandInterpreter {
    ...
    void createStatement() {
        ...
        String name = name();
        Table table = tableDefinition();
        _database.put(name,table);  // FILL IN CODE TO EXECUTE THE STATEMENT
        _input.next(";");
    }
    ...
    void printStatement() {
        ...
        String name = _input.peek();
        Table table = tableName();
        System.out.printf("Contents of %s.db%n", name);
        _input.next(";");
        table.print();
    }
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            newtable = table1.select(table2, list, conditions);
        } else {
            conditions = conditionClause(table1);
            newtable = table1.select(list, conditions);
        }
        return newtable;
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        if (_input.nextIf("where")) {
            Condition condition1 = condition(tables);
            conditionList.add(condition1);
            while (_input.nextIf("and")) {
                Condition conditionother = condition(tables);
                conditionList.add(conditionother);
            }
        }
    ...
    Condition condition(Table... tables) {
        Column acolumn = new Column(columnName(), tables);
        String arelation = _input.next();
        Condition condition1;
        if (_input.nextIs(Tokenizer.IDENTIFIER)) {
            Column bcolumn = new Column(columnName(), tables);
            condition1 = new Condition(acolumn, arelation, bcolumn);
        } else {
            condition1 = new Condition(acolumn, arelation, literal());
        }
        return condition1;
    }
